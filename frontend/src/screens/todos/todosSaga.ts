...
function* getTodosRequest(): Generator<
  any,
  any,
  AxiosResponse<TodoType[] | string>
> {
  try {
    const response: AxiosResponse<TodoType[] | string> = yield call(() =>
      api.get<TodoType[]>(`/todos`)
    )
    ...
    yield put(todosActions.getTodosFailed(errors as string))
  }
}
...
function* deleteTodoRequest(
  action: any
): Generator<any, any, AxiosResponse<TodoType[] | string>> {
  try {
    const response: AxiosResponse<TodoType[] | string> = yield call(() =>
    ...
    yield put(todosActions.deleteTodoFailed(errors as string))
  }
}
...
function* createTodoRequest(
  ...
  try {
    ...
    yield put(todosActions.createTodoFailed(errors as string))
  }
}
...
function* updateTodoRequest(
  ...
  try {
    const response: AxiosResponse<TodoType | string> = yield call(() =>
      api.put(`/todos/${action.payload.id}`, action.payload)
    )
    if (response.status === StatusCode.Success) {
    ...
    yield put(todosActions.updateTodoFailed(errors as string))
  }
}
function* todosSaga(): Generator<any, any, any> {
  return [
